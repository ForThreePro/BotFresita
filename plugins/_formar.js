...
let handler = async (m, { conn, groupMetadata }) => {
  ...
  if (participantes.length < 4) {
    return m.reply('🍓 *BOT FRESITA - CUPIDO* 🍓\n\nAy nooo me faltan fresitas 🥺👉👈\nNecesito mínimo 4 en el grupo para shippear 2 parejitas 💘')
  }
  ...
  // Armar el mensaje
  let texto = `🍓 *BOT FRESITA - MODO CUPIDO* 🍓\n\n`
  texto += `✨ *Activando chismecito dulce y vibras rosita* ✨\n\n`

  for (let i = 0; i < parejas.length; i++) {
    let [p1, p2] = parejas[i]
    let frase = frasesAmor[Math.floor(Math.random() * frasesAmor.length)]

    texto += `💕 *SHIP FRESITA #${i+1}* 💕\n`
    texto += `👥 @${p1.split('@')[0]} + @${p2.split('@')[0]}\n`
    texto += `📝 _${frase}_\n\n`
  }

  texto += `🌸 Que nazca el amor... o mínimo risas para el grupo 🌸\n`
  texto += `🍓 Hecho con mucho amor por *Bot Fresita* 🍓`
