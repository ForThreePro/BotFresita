...
let handler = async (m, { conn, text }) => {
  ...
  if (!text ||!text.includes('/')) {
    return m.reply(`🍓╭─〔 *BOT FRESITA - CAMBIOS* 〕─╮🍓
│
│ 🍰 *COMO USAR:*
...
🍓╰───────────────╯🍓`)
  ...
  if (isNaN(monto) || monto <= 0) return m.reply(`🍓 💔 *Fresita dice: Monto inválido* 🍓`)
  if (de.length!== 3 || a.length!== 3) return m.reply(`🍓 💔 *Fresita dice: Usa 3 letras* PEN, USD, ARS 🍓`)
...
> ${MARCA} | Datos frescos 🍓`
    ...
    console.log(e)
    m.reply(`🍓 💔 *Fresita dice: ERROR* ${e.message}\n\nRevisa: PEN, USD, ARS, COP... 🍓`)
  }
}
